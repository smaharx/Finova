...
@router.post("/{transaction_id}/correction", response_model=dict)
def teach_ai_transaction(
    transaction_id: int, item: TransactionCorrectionInput, db: Session = Depends(get_db)
):
    record = (
    ...
    try:
        ...
        record.is_anomaly = check_for_anomaly(
            db, corrected_category, record.amount, exclude_id=record.id
        )
